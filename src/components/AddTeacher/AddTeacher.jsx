...
class AddTeacher extends Component {
  ...
  addStudent = async e => {
    e.preventDefault();
    try {
      const newTeacher = await axios.post("/api/teachers/", {
	  ...
	  toast("Teacher " + newTeacher.data.newTeacher.name + " created successfully" ,{ type: toast.TYPE.SUCCESS, autoClose: 3000 });
    } catch (err) {
    ...
    return (
      <div className="AddStudent-Wrapper">
        ...
        <form onSubmit={this.addStudent}>
          ...
          <input
            type="text"
            placeholder="Enter the name of the new teacher here"
            name="name"
          ...
          <input
            type="text"
            placeholder="enter teacher email here"
            name="email"
            onChange={this.onChangeHandler}
            ref="email"
